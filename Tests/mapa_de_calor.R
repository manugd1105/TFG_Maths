...
B_boot  <- 100   # Número de replicaciones bootstrap que hace gamma.test
...
Potencia <- function(mu,sigma){
  rechazos <- replicate(n_rep,{
    Nest <- rnorm(n)
    N <- rnorm(n,mu,sigma)
    gamma.test(Nest,N,gamma0_less,boot=T,B=B_boot,alternative="less")$p_value<=alpha
  })
...
Potencia <- function(mu,sigma){
  rechazos <- replicate(n_rep,{
    Nest <- rnorm(n)
    N <- rnorm(n,mu,sigma)
    pi.test(N,Nest,pi0,B=B_boot,alternative="greater")$p_value<=alpha
  })
  return(mean(rechazos))
}
...
Nest <- rnorm(n)
N <- rnorm(n,mu,sigma)
gamma[i] <- gamma.test(Nest,N,gamma0_less,boot=F,B=B_boot,alternative="less")$gamma
}
return(mean(gamma))
}
...
pip <- function(mu,sigma) {
  ...
  for (i in 1:100) {
    Nest <- rnorm(n)
    N <- rnorm(n,mu,sigma)
    gamma[i] <- pi.test(Nest,N,pi0,B=B_boot,alternative="less")$pi
  }
  return(mean(gamma))
}
...
gam <- function(mu,sigma) {
  gamma <- pnorm(mu/(abs(1-sigma)))
  return(gamma)
}

gam <- Vectorize(gam)
df.heatmapN$Indice <-gam(df.heatmapN$mu,df.heatmapN$sigma)
ggplot(df.heatmapN, aes(x = mu, y = sigma, fill = R)) + geom_tile() + 
  scale_fill_gradient2(low = "#FF0000", high = "#006600", mid = "white", midpoint = 0.5) +
  # scale_fill_gradient2(low = "#666666", high = "black", mid = "white", midpoint = 0.5) +
  labs(x="\u03BC", y="\u03C3") + theme(axis.title.y = element_text(angle = 0, vjust = 0.5))
...
alpha   <- 0.05  # Nivel de significación
B_boot  <- 100   # Número de replicaciones bootstrap que hace gamma.test
eps <- 0.13 # Proporcion de la muestra de outliers
...
Potencia <- function(mu,sigma){
  rechazos <- replicate(n_rep,{
    ...
    idx <- sample(n, n_out)
    half <- floor(n_out/2)
    N[idx] <- c(rep(-10^10, half),
                   rep(10^10 * sd(N), n_out - half))
    gamma.test(Nest,N,gamma0_less,boot=F,B=B_boot,alternative="less")$p_value<=alpha
  })
  return(mean(rechazos))
}
Potencia <- Vectorize(Potencia)
...
alpha   <- 0.05  # Nivel de significación
B_boot  <- 100   # Número de replicaciones bootstrap que hace gamma.test
eps <- 0.05 # Proporcion de la muestra de outliers
k1 <- 7 # Factor del shift de la muestra X
k2 <- 7 # Factor del shift de la muestra Y
...
Potencia <- function(mu,sigma){
  rechazos <- replicate(n_rep,{
    ...
    idx <- sample(n, n_out)
    half <- floor(n_out/2)
    N[idx] <- c(rep(k2 * sd(N), half),
                rep(-k2 * sd(N), n_out - half))
